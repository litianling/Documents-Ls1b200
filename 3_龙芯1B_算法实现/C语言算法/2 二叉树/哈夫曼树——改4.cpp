...
int main()
{
	/* 待编码数据数组 */
	int number = 6;
	int basic_date[6]={5,9,12,13,16,45};
	char putchar[7]={'a','b','c','d','e','f'};

	/* 构建哈夫曼树 */					// 哈夫曼树的实际节点总数为2*number-1
	int HF_Tree[48]={0};				// 如果我们创建结构体数组，一个数组元素中存放一个蛤目鳗节点则共需要开辟
	...
	for (int i = 1; i <= number; i++)
	{
		printf("字符：<%c> 频率：<%d>	编码:", putchar[i-1],HF_Tree[i*4]);
		for(int j=0;j<6;j++)
