...
UINT8C key_code_map[6] = {
	0x31,0x32,0x33,0x34,0x35,0x36         //按键1,按键2,按键3,按键4,按键3,按键4
};
/*普通键盘数据*/
...
void EC16Drive()
{
	EC16_A_State = EC16_A;	//读取EC16_A状态
	if(EC16_A_State != EC16_A_PState)
	{
	   if(EC16_A_State == 1) //顺时针旋转
	   {
	      if(EC16_B == 0) //判断EC16_B状态
		  {
		  	  KeyAction(key_code_map[4]);
		  }
		  else
		  {
			  KeyAction(key_code_map[5]);
		  }
	   }
	   else	//逆时针旋转
	   {
		  if(EC16_B  == 1)//判断EC16_B状态
		  {
		  	  KeyAction(key_code_map[4]);
		  }
		  else
