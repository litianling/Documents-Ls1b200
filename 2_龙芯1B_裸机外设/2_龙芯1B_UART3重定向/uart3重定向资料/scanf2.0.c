...
#define FL_NOASSIGN         (1U <<  0U)  //1
#define FL_WIDTHSPEC        (1U <<  1U)  //2
#define FL_SHORT            (1U <<  2U)  //8
#define FL_LONG             (1U <<  3U)  //16
#define FL_LONGDOUBLE       (1U <<  4U)  //32
...
int _doscan(const char *format, va_list ap)
{
    ...
    while (1)
    {
        ...
        {
            format++;
            flags |= FL_NOASSIGN;// 相应标志位变化（按位或，置一）
        }
